...
int main(void) {
    ...
    *(video_in_dma+3) = 0x0; 
    ...
    unsigned char bwScaled[28 * 28];


    static unsigned char grayPixels[IMAGE_HEIGHT][IMAGE_WIDTH];
    uint64_t sumGray = 0;

    for(int y = 0; y < IMAGE_HEIGHT; ++y) {
      for(int x = 0; x < IMAGE_WIDTH; ++x) {
        unsigned short raw = video_mem[(y<<9) + x];
        unsigned char r5 = (raw >> 11) & 0x1F;
        unsigned char g6 = (raw >>  5) & 0x3F;
        unsigned char b5 =  raw        & 0x1F;
        unsigned char gray = (r5 << 3) * 0.299f
                           + (g6 << 2) * 0.587f
                           + (b5 << 3) * 0.114f;
        grayPixels[y][x] = gray;
        sumGray += gray;
   ...
   unsigned char avgGray = sumGray / (IMAGE_WIDTH * IMAGE_HEIGHT);

    for(int y = 0; y < IMAGE_HEIGHT; ++y) {
      for(int x = 0; x < IMAGE_WIDTH; ++x) {
        bwPixels[y][x] = (grayPixels[y][x] > avgGray) ? 0xFF : 0x00;
      }
    }

    updated_scaleImagePreservingAspectRatio(
        &bwPixels[0][0],
        bwScaled,
         320, 240,
         28, 28 
    ...
    saveImageGrayscale(filename1,bwScaled,28,28)
